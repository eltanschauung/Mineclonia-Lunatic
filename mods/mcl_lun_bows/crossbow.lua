...
mcl_lun_bows.CROSSBOW_CHARGE_TIME_HALF = BOW_CHARGE_TIME_HALF / 1e+6
mcl_lun_bows.CROSSBOW_CHARGE_TIME_FULL = BOW_CHARGE_TIME_FULL / 1e+6
...
local function play_load_sound(id, object)
	core.sound_play({name = "mcl_lun_bows_crossbow_drawback_"..id, gain=0.3}, {object=object, max_hear_distance=16}, true)
end
...
function mcl_lun_bows.shoot_arrow_crossbow (arrow_item, pos, dir, yaw, shooter, speed, damage, is_critical, crossbow_stack, collectable)
	local has_multishot_enchantment
		= crossbow_stack and mcl_enchanting.has_enchantment (crossbow_stack, "multishot")
	core.sound_play({name="mcl_lun_bows_crossbow_shoot", gain=0.035}, {pos=pos, max_hear_distance=32}, true)
	local inaccuracy = (shooter and shooter:is_player() and 1) or nil -- No dispenser; that use mcl_lun_bows.shoot_arrow
	dir = mcl_lun_bows.add_inaccuracy(dir, inaccuracy)
...
function mcl_lun_bows.get_arrow_stack_for_crossbow (player)
...
local function player_shoot_arrow(wielditem, player, is_critical)
	...
	mcl_lun_bows.shoot_arrow_crossbow (arrow_itemstring, playerpos, dir, yaw, player, BOW_MAX_SPEED, nil, is_critical, player:get_wielded_item(), true)
	return true
end

-- Bow item, uncharged state
core.register_tool("mcl_lun_bows:crossbow", {
...
S("The speed and damage of the arrow increases the longer you charge. The regular damage of the arrow is between 1 and 9. At full charge, there's also a 20% of a critical hit, dealing 10 damage instead."),
	...
	inventory_image = "mcl_lun_bows_crossbow.png",
...
core.register_tool("mcl_lun_bows:crossbow_loaded", {
	description = S("Crossbow"),
	_tt_help = S("Launches arrows"),
	_doc_items_longdesc = S("Crossbows are ranged weapons to shoot arrows at your foes.").."\n"..
S("The speed and damage of the arrow increases the longer you charge. The regular damage of the arrow is between 1 and 9. At full charge, there's also a 20% of a critical hit, dealing 10 damage instead."),
	...
	_doc_items_durability = BOW_DURABILITY,
	inventory_image = "mcl_lun_bows_crossbow_3.png",
	wield_scale = mcl_vars.tool_wield_scale,
	stack_max = 1,
	range = 4,
	-- Trick to disable digging as well
...
local function reset_bows(player)
	...
	for place, stack in pairs(list) do
		if stack:get_name() == "mcl_lun_bows:crossbow" or stack:get_name() == "mcl_lun_bows:crossbow_enchanted" then
			stack:get_meta():set_string("active", "")
		elseif stack:get_name()=="mcl_lun_bows:crossbow_0" or stack:get_name()=="mcl_lun_bows:crossbow_1" or stack:get_name()=="mcl_lun_bows:crossbow_2" then
			stack:set_name("mcl_lun_bows:crossbow")
			stack:get_meta():set_string("active", "")
			list[place] = stack
		elseif stack:get_name()=="mcl_lun_bows:crossbow_0_enchanted" or stack:get_name()=="mcl_lun_bows:crossbow_1_enchanted" or stack:get_name()=="mcl_lun_bows:crossbow_2_enchanted" then
			stack:set_name("mcl_lun_bows:crossbow_enchanted")
			stack:get_meta():set_string("active", "")
			list[place] = stack
		end
	end
...
local function reset_bow_state(player, also_reset_bows)
	...
	if core.get_modpath("playerphysics") then
		playerphysics.remove_physics_factor(player, "speed", "mcl_lun_bows:use_crossbow")
	end
...
for level=0, 2 do
	core.register_tool("mcl_lun_bows:crossbow_"..level, {
		description = S("Crossbow"),
		_doc_items_create_entry = false,
		inventory_image = "mcl_lun_bows_crossbow_"..level..".png",
		wield_scale = mcl_vars.tool_wield_scale,
		stack_max = 1,
		...
		on_drop = function(itemstack, dropper, pos)
			...
			if mcl_enchanting.is_enchanted(itemstack:get_name()) then
				itemstack:set_name("mcl_lun_bows:crossbow_enchanted")
			else
				itemstack:set_name("mcl_lun_bows:crossbow")
			end
...
function mcl_lun_bows.crossbow_charge_time_multiplier (quick_charge)
	local time = _BOW_CHARGE_TIME_FULL - (quick_charge * 0.13 * 1.0e+6)
	return math.max (0.0, time / _BOW_CHARGE_TIME_FULL)
end

function mcl_lun_bows.load_crossbow (player, wielditem, usetime)
	...
	if core.is_creative_enabled(player:get_player_name()) then
		if arrow_stack then
			...
			arrow_itemstring = "mcl_lun_bows:arrow"
		end
	else
		arrow_itemstring = arrow_stack:get_name()
	...
	if not mcl_enchanting.is_enchanted (wielditem:get_name ()) then
		wielditem:set_name("mcl_lun_bows:crossbow_loaded")
	else
		wielditem:set_name("mcl_lun_bows:crossbow_loaded_enchanted")
	end
...
controls.register_on_release(function(player, key)
	...
	if fully_drawn(wielditem:get_name()) and (creative or arrow_stack) then
		...
		if wielditem:get_name()=="mcl_lun_bows:crossbow_2" then
			wielditem:set_name("mcl_lun_bows:crossbow_loaded")
		else
			wielditem:set_name("mcl_lun_bows:crossbow_loaded_enchanted")
		end
		player:set_wielded_item(wielditem)
		core.sound_play({name="mcl_lun_bows_crossbow_load", gain=0.3}, {object=player, max_hear_distance=16}, true)
	else
...
controls.register_on_press(function(player, key)
	if key~="LMB" then return end
		local wielditem = player:get_wielded_item()
		if wielditem:get_name()=="mcl_lun_bows:crossbow_loaded" or wielditem:get_name()=="mcl_lun_bows:crossbow_loaded_enchanted" then
		local enchanted = mcl_enchanting.is_enchanted(wielditem:get_name())
		local has_shot = player_shoot_arrow (wielditem, player, true)

		if enchanted then
			wielditem:set_name("mcl_lun_bows:crossbow_enchanted")
		else
			wielditem:set_name("mcl_lun_bows:crossbow")
		end
...
controls.register_on_hold(function(player, key)
	...
	if bow_load[name] == nil
		and (wielditem:get_name()=="mcl_lun_bows:crossbow" or wielditem:get_name()=="mcl_lun_bows:crossbow_enchanted")
		and (wielditem:get_meta():get("active") or key=="zoom") and (creative or get_arrow(player)) then
			local enchanted = mcl_enchanting.is_enchanted(wielditem:get_name())
			if enchanted then
				wielditem:set_name("mcl_lun_bows:crossbow_0_enchanted")
				play_load_sound(0, player)
			else
				wielditem:set_name("mcl_lun_bows:crossbow_0")
				play_load_sound(0, player)
			end
			player:set_wielded_item(wielditem)
			if core.get_modpath("playerphysics") then
				-- Slow player down when using bow
				playerphysics.add_physics_factor(player, "speed", "mcl_lun_bows:use_crossbow", PLAYER_USE_CROSSBOW_SPEED)
			end
		...
		if player:get_wield_index() == bow_index[name] then
			if type(bow_load[name]) == "number" then
				if wielditem:get_name() == "mcl_lun_bows:crossbow_0" and core.get_us_time() - bow_load[name] >= BOW_CHARGE_TIME_HALF then
					wielditem:set_name("mcl_lun_bows:crossbow_1")
					play_load_sound(1, player)
				elseif wielditem:get_name() == "mcl_lun_bows:crossbow_0_enchanted" and core.get_us_time() - bow_load[name] >= BOW_CHARGE_TIME_HALF then
					wielditem:set_name("mcl_lun_bows:crossbow_1_enchanted")
					play_load_sound(1, player)
				elseif wielditem:get_name() == "mcl_lun_bows:crossbow_1" and core.get_us_time() - bow_load[name] >= BOW_CHARGE_TIME_FULL then
					wielditem:set_name("mcl_lun_bows:crossbow_2")
					play_load_sound(2, player)
				elseif wielditem:get_name() == "mcl_lun_bows:crossbow_1_enchanted" and core.get_us_time() - bow_load[name] >= BOW_CHARGE_TIME_FULL then
					wielditem:set_name("mcl_lun_bows:crossbow_2_enchanted")
					play_load_sound(2, player)
				end
			else
				if wielditem:get_name() == "mcl_lun_bows:crossbow_0" or wielditem:get_name() == "mcl_lun_bows:crossbow_1" or wielditem:get_name() == "mcl_lun_bows:crossbow_2" then
					wielditem:set_name("mcl_lun_bows:crossbow")
					play_load_sound(1, player)
				elseif wielditem:get_name() == "mcl_lun_bows:crossbow_0_enchanted" or wielditem:get_name() == "mcl_lun_bows:crossbow_1_enchanted" or wielditem:get_name() == "mcl_lun_bows:crossbow_2_enchanted" then
					wielditem:set_name("mcl_lun_bows:crossbow_enchanted")
					play_load_sound(1, player)
				end
			end
...
mcl_player.register_globalstep(function(player)
	if not mcl_serverplayer.is_csm_capable (player) then
		...
		if type(bow_load[name]) == "number" and ((wielditem:get_name()~="mcl_lun_bows:crossbow_0" and wielditem:get_name()~="mcl_lun_bows:crossbow_1" and wielditem:get_name()~="mcl_lun_bows:crossbow_2" and wielditem:get_name()~="mcl_lun_bows:crossbow_0_enchanted" and wielditem:get_name()~="mcl_lun_bows:crossbow_1_enchanted" and wielditem:get_name()~="mcl_lun_bows:crossbow_2_enchanted") or wieldindex ~= bow_index[name]) then
			reset_bow_state(player, true)
		end
	end
end)
...
-- No crafting recipe for this crossbow in this game.

-- Add entry aliases for the Help
if core.get_modpath("doc") then
	doc.add_entry_alias("tools", "mcl_lun_bows:crossbow", "tools", "mcl_lun_bows:crossbow_0")
	doc.add_entry_alias("tools", "mcl_lun_bows:crossbow", "tools", "mcl_lun_bows:crossbow_1")
	doc.add_entry_alias("tools", "mcl_lun_bows:crossbow", "tools", "mcl_lun_bows:crossbow_2")
end
